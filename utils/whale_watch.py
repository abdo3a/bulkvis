...
def main():
    ...
    if args.debug:
        debug(args)
    summary_df = combine_df(args.summary)
    fused_df, ss, fused_read_ids = fuse_reads(summary_df, args.paf, args.distance, args.top, args.alt, args.debug)
...
def fuse_reads(ss, paf, distance, top_N, alt, debug):
    """
...
def combine_df(dfs):
    """given a file list, combine them into a single dataframe and return it"""
    df_list = []
    fields = ['channel', 'start_time', 'duration', 'run_id', 'read_id', 'sequence_length_template', 'filename']
    # ss = pd.read_csv(sequencing_summary, sep='\t', usecols=ss_fields)
    for fp in dfs:
        df_list.append(pd.read_csv(full_path(fp), sep='\t', usecols=fields))
